<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            {{ __('messages.suppliers_crm') }}
        </h2>
            <a href="{{ route('suppliers.create') }}" class="btn-premium">
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            ...
            <div class="glass-card overflow-hidden animate-fade-in-up">
                <div class="p-4 sm:p-6 border-b border-slate-200 dark:border-slate-700/50 flex flex-col lg:flex-row justify-between items-start lg:items-center gap-4">
                    <h3 class="text-xl font-bold text-slate-900 dark:text-white">{{ __('messages.supplier_list') }}</h3>
                    ...
                    <div class="flex flex-col sm:flex-row gap-2 w-full lg:w-auto">
                        ...
                        <a href="{{ route('suppliers.create') }}" class="btn-premium !py-2 !px-4 !rounded-xl !bg-indigo-600 hover:!bg-indigo-700 !border-none flex justify-center items-center gap-1 shadow-md shadow-indigo-500/20 w-full sm:w-auto">
                            + {{ __('messages.add_supplier') }}
                        </a>
                    </div>
                </div>
                    ...
                    <table class="w-full text-left border-collapse data-table">
                        <thead>
                            <tr class="bg-slate-50 dark:bg-slate-900/30 text-slate-500 dark:text-text-muted text-sm uppercase tracking-wider">
                                ...
                                <th class="p-4 font-medium text-left">{{ __('messages.company_name') }}</th>
                                <th class="p-4 font-medium text-left">{{ __('messages.contact_person') }}</th>
                                <th class="p-4 font-medium text-left">{{ __('messages.contact_info') }}</th>
                        ...
                        <tbody class="text-sm divide-y divide-slate-200 dark:divide-slate-700/50">
                            ...
                            <tr class="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                                <td class="p-4 text-slate-500 dark:text-text-muted text-left">#{{ $supplier->id }}</td>
                                <td class="p-4 text-slate-900 dark:text-white font-medium text-left">{{ $supplier->name }}</td>
                                <td class="p-4 text-slate-500 dark:text-text-muted text-left">{{ $supplier->contact_person ?? '-' }}</td>
                                <td class="p-4 text-slate-500 dark:text-text-muted text-left">
                                ...
                                <td class="p-4 text-right">
                                    <div class="flex gap-2 justify-end">
                                        <a href="{{ route('suppliers.edit', $supplier) }}" class="text-brand-primary hover:text-brand-accent transition-colors">{{ __('messages.edit') }}</a>
                                        <form action="{{ route('suppliers.destroy', $supplier) }}" method="POST" class="inline" onsubmit="return confirm('{{ __('messages.confirm_delete_supplier') }}');">
                                            @csrf
                                            @method('DELETE')
