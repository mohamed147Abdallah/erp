            ...
            <div class="glass-card p-8 animate-fade-in-up">
                ...
                <form action="{{ route('admin.users.store') }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 md:grid-cols-2 gap-6 mb-6">
                        ...
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.password_star') }}</label>
                            <input type="password" name="password" class="input-premium" required>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.confirm_password_star') }}</label>
                            <input type="password" name="password_confirmation" class="input-premium" required>
                        </div>
                            ...
                            <div class="grid grid-cols-2 md:grid-cols-3 gap-4 bg-slate-50 dark:bg-slate-900/30 p-4 rounded-xl border border-slate-200 dark:border-slate-700/50">
                                @forelse($roles as $role)
                                    <label class="flex items-center space-x-3 cursor-pointer">
                                        <input type="checkbox" name="roles[]" value="{{ $role->name }}" class="form-checkbox h-5 w-5 text-brand-primary rounded border-slate-300 focus:ring-brand-primary dark:border-slate-600 dark:bg-slate-800">
                                        <span class="text-slate-700 dark:text-slate-300 font-medium">{{ $role->name }}</span>
                                    </label>
                                @empty
                                    <p class="text-slate-500 dark:text-slate-400 text-sm col-span-full">{{ __('messages.no_roles_available') }}</p>
                                @endforelse
                            </div>
                        </div>
                    </div>
