    ...
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            <h2 class="font-bold text-2xl text-slate-900 dark:text-white tracking-wide">
                {{ __('{{ __('messages.edit_category') }}: ') }} {{ $category->name }}
            </h2>
                ...
                <form action="{{ route('categories.update', $category) }}" method="POST">
                    ...
                    <div class="grid grid-cols-1 gap-6 mb-6">
                        ...
                        <div>
                            ...
                            <textarea name="description" rows="4" class="input-premium">{{ old('description', $category->description) }}</textarea>
                        </div>
                        <div class="flex items-center mt-2">
                            <input type="checkbox" name="is_active" id="is_active" class="rounded border-slate-300 text-brand-primary shadow-sm focus:ring-brand-primary dark:bg-slate-800 dark:border-slate-600 h-5 w-5" {{ old('is_active', $category->is_active) ? 'checked' : '' }}>
