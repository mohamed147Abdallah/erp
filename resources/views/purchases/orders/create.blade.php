<x-app-layout>
    <x-slot name="header">
        <div class="flex flex-col md:flex-row justify-between items-start md:items-center gap-4">
            ...
            <a href="{{ route('purchases.orders.index') }}" class="text-slate-500 hover:text-slate-700 dark:text-slate-400 dark:hover:text-slate-300 transition-colors">
                &larr; {{ __('messages.back_to_pos') }}
            </a>
        </div>
    </x-slot>
    ...
    <div class="py-12">
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8">
            <div class="glass-card p-8 animate-fade-in-up">
                ...
                <form action="{{ route('purchases.orders.store') }}" method="POST" id="poForm">
                    @csrf
                    
                    <div class="grid grid-cols-1 md:grid-cols-3 gap-6 mb-8 pb-8 border-b border-slate-700/50">
                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.po_number_star') }}</label>
                            <input type="text" name="order_number" value="{{ old('order_number', $orderNumber) }}" class="input-premium" required readonly>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.supplier_star') }}</label>
                            <select name="supplier_id" class="input-premium" required>
                        ...
                        <div>
                            ...
                            <select name="status" class="input-premium" required>
                                ...
                                <option value="ordered" {{ old('status') == 'ordered' ? 'selected' : '' }}>{{ __('messages.ordered') }}</option>
                                <option value="received" {{ old('status') == 'received' ? 'selected' : '' }}>{{ __('messages.received_adds_stock') }}</option>
                            </select>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.order_date_star') }}</label>
                            <input type="date" name="order_date" value="{{ old('order_date', date('Y-m-d')) }}" class="input-premium" required>
                        </div>

                        <div>
                            <label class="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-2">{{ __('messages.expected_delivery_date') }}</label>
                            <input type="date" name="expected_delivery_date" value="{{ old('expected_delivery_date') }}" class="input-premium">
                        </div>
                    </div>

                    <div class="mb-8">
                        <h3 class="text-lg font-bold text-slate-900 dark:text-white mb-4">{{ __('messages.order_items') }}</h3>
                            ...
                            <table class="w-full text-left border-collapse data-table" id="itemsTable">
                                <thead>
                                    <tr class="bg-slate-50 dark:bg-slate-900/30 text-slate-500 dark:text-text-muted text-sm uppercase tracking-wider">
                                        ...
                                        <th class="p-3 font-medium text-left w-1/6">{{ __('messages.unit_cost') }}</th>
    ...
    <script>
        ...
        function removeRow(btn) {
            ...
            if (document.querySelectorAll('.item-row').length > 1) {
                ...
                alert("You must have at least one item on the order.");
            }
        }
